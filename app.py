...
def _inject_css():
    ...
    if mode == "claro":
        HB_BG, HB_CARD = "#f3f4f6", "#ffffff"
        HB_TEXT = "#0f172a"
    else:
        HB_BG, HB_CARD = "#0f1116", "#141821"
        HB_TEXT = "#f8fafc"
    st.markdown(f"""
<style>
...
def _ensure_clientes_obras_extra(engine):
...
_ensure_clientes_obras_extra(engine)
...
def buscar_cnpj_endereco(cnpj: str) -> dict | None:
    ...
    for url in urls:
        try:
            ...
            razao = data.get("razao_social") or data.get("nome") or data.get("razao") or data.get("nome_empresarial") or ""
            ...
            municipio = data.get("municipio") or data.get("cidade") or est.get("cidade_nome") or est.get("cidade") or ""
            ...
            telefone = data.get("telefone") or est.get("telefone") or est.get("telefone1") or ""
            return {
                "razao_social": razao,
...
# ======================================================================
# CALLBACKS (para não dar StreamlitAPIException)
# ======================================================================
...
def _on_page(canvas, doc, titulo: str = ""):
    ...
    foot = f"Habisolute Engenharia — {FORM_CODE}  {agora}  pág. {pagina}"
    canvas.setFont("Helvetica", 8.5)
    tw = canvas.stringWidth(foot, "Helvetica", 8.5)
    canvas.setFillColor(colors.black)
    canvas.drawString((w - tw) / 2.0, 15, foot)
    canvas.restoreState()
...
def gerar_pdf_medicao(obra_nome: str, periodo_str: str, linhas: list[dict], logo_bytes: bytes | None, medicao_num: int, signature_bytes: bytes | None = None) -> bytes:
    ...
    tit_tbl = Table([[Paragraph(f"<b>{titulo}</b>", ParagraphStyle("tm", parent=styleN, fontSize=11, alignment=TA_CENTER))]], colWidths=[doc.width])
    tit_tbl.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1), colors.HexColor("#e6e6e6")),("BOX",(0,0),(-1,-1),0.5,colors.black)]))
    story += [tit_tbl, Spacer(1,6)]
...
def page_obras():
    ...
    with col_form:
        ...
        # EDITAR
            ...
            tab_dados, tab_anexos, tab_servs = st.tabs(["Dados", "Anexos", "Serviços"])
            with tab_dados:
                ...
                if st.button("Salvar alterações", key=f"btn_save_obra_{obra_edit.id}"):
                    ...
                    _rerun()
            with tab_anexos:
                st.write("Envie documentos da obra")
            ...
            with tab_servs:
                st.write("Serviços específicos da obra")
                with SessionLocal() as sess:
                ...
                with c3:
                    ...
                    if st.button("Vincular/atualizar serviço", key="btn_vinc_srv"):
                        ...
                        with SessionLocal() as sess:
                            ...
                            if osrv:
                                osrv.preco_unit = preco_espec; osrv.ativo = 1
                            else:
                ...
                if obra_servs:
                    df_os = pd.DataFrame([{
                        "Serviço": next((f"{s.codigo} — {s.descricao}" for s in servicos_all if s.id==osrv.servico_id), osrv.servico_id),
                        "Preço específico": osrv.preco_unit or 0.0
        ...
        if sel != "(Novo serviço)":
            ...
            if st.button("Salvar serviço"):
                with SessionLocal() as sess:
                    ...
                    s2.codigo = codigo; s2.descricao = desc; s2.unidade = un; s2.preco_unit = preco; s2.ativo = 1 if ativo else 0
                    sess.commit()
            ...
            if st.button("Criar serviço"):
                with SessionLocal() as sess:
                    sess.add(Servico(codigo=codigo, descricao=desc, unidade=un, preco_unit=preco, ativo=1))
                    sess.commit()
